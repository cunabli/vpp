...
typedef struct
{
  ...
  /* Deferred-teardown observability: a session freed while SEC still
   ...
   * `show dpaa2 ipsec` -- deferred should equal drained once traffic quiesces. */
  u64 sessions_deferred; /* teardowns deferred because ops were in flight;
			    main-thread only, mutated under the worker barrier */
  u64 sessions_drained;  /* deferred teardowns the poll node later completed;
			    atomic RMW -- any qp-owning worker increments it */
...
/* ESP offload provider callbacks (defined in the session file, registered by
 * init together with the demux node indices). */
int dpaa2_ipsec_check_support (ipsec_sa_t *sa);
void dpaa2_ipsec_session_add_del (u32 sa_index, int is_add);
...
typedef struct
{
  ...
  u64 straggler;    /* completions for a deleted SA (dropped, any status) */
