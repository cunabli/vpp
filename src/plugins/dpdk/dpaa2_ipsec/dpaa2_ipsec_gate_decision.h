/*
 * dpaa2_ipsec_gate_decision.h - pure SA routing predicate (offload vs fallback)
 *
 * Freestanding: no VPP or DPDK includes, so the routing decision is unit-testable
 * on the host with plain cc (see test_dpaa2_ipsec_gate.c). The plugin's
...
/* Why an SA is on the fallback path -- surfaced by "show ipsec offload" to prove
 ...
 * by traffic, which is the security invariant behind ADR 0006 (a)/(b). If a size
