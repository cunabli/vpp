...
dpaa2_ipsec_offload_gate (ipsec_sa_t *sa, dpaa2_ipsec_fallback_reason_t *reason)
{
  ...
  *reason = DPAA2_IPSEC_FALLBACK_NONE;
   ...
   * (a dynamic, per-device gate). Algorithm must map to a session we build. */
  if (build_crypto_xform (&a, &b, sa, 1 /* is_out, just to validate */) == 0)
    {
      *reason = DPAA2_IPSEC_FALLBACK_UNSUPPORTED_CONFIG;
      return 0;
    }
  /* Device anti-replay window tops out at 1024; larger requested windows run
   * on fallback with the full window rather than being silently truncated. */
  if (sa_anti_replay_window (sa) > 1024)
    {
