/*
 * test_dpaa2_ipsec_gate.c - host unit test for the pure SA routing predicate.
 *
 * Board-free: the decision core has no VPP/DPDK deps, so this compiles and runs
 * with plain cc. Covers the ADR 0006 seeds T1 (no size input), T2 (disable
 * overrides capability), T4 (reason-string table parity), plus the window
 * boundary and device-present gates.
 *
 *   cc -Wall -Werror test_dpaa2_ipsec_gate.c && ./a.out
 *
