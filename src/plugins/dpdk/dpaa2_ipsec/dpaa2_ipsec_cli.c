 ...
 *
 * `show ipsec offload` is the summary: which devices can offload, how workers
 * map to queue-pairs, and plugin-wide packet totals (offloaded vs fallback vs
 * dropped). `show ipsec offload verbose` adds a per-SA line -- offload vs
 ...
 * nodes feed; the offload nodes now feed it too (see dpaa2_ipsec_node.c), so an
...
VLIB_CLI_COMMAND (dpaa2_ipsec_show_offload_cmd, static) = {
  .path = "show ipsec offload",
  .short_help = "show ipsec offload [verbose]",
