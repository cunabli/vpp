...
typedef void (vlib_buffer_pool_free_fn_t) (struct vlib_main_t *vm,
					   struct vlib_buffer_pool_t_ *bp,
					   u32 *buffers, u32 n_buffers);
typedef u32 (vlib_buffer_pool_count_fn_t) (struct vlib_main_t *vm,
					   struct vlib_buffer_pool_t_ *bp);
...
typedef struct
{
  ...
  /* optional: how many buffers the backend can still supply.  When set,
     `show buffers` and the stats gauges report it instead of n_avail, which
     a backend-owned pool deliberately keeps at zero. */
  vlib_buffer_pool_count_fn_t *count;
